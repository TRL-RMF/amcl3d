...
void Node::spin()
{
  ...
  pose_pub_ = nh_.advertise<geometry_msgs::PoseWithCovarianceStamped>("amcl3d_pose", 10);

  cloud_filter_pub_ = nh_.advertise<sensor_msgs::PointCloud2>("pointcloud_filtered", 1);
...
void Node::odomCallback(const geometry_msgs::TransformStampedConstPtr& msg)
{
  ...
  tf_br.sendTransform(tf::StampedTransform(lastodom_2_world_tf_, ros::Time::now() + ros::Duration(1.0), parameters_.global_frame_id_,
                                           parameters_.odom_frame_id_));

  // publish pose
  geometry_msgs::PoseWithCovarianceStamped pose;
  pose.header.stamp = msg->header.stamp;
  pose.header.frame_id = parameters_.global_frame_id_;
  pose.pose.pose.position.x = lastbase_2_world_tf_.getOrigin().getX();
  pose.pose.pose.position.y = lastbase_2_world_tf_.getOrigin().getY();
  pose.pose.pose.position.z = lastbase_2_world_tf_.getOrigin().getZ();
  pose.pose.pose.orientation.x = lastbase_2_world_tf_.getRotation().getX();
  pose.pose.pose.orientation.y = lastbase_2_world_tf_.getRotation().getY();
  pose.pose.pose.orientation.z = lastbase_2_world_tf_.getRotation().getZ();
  pose.pose.pose.orientation.w = lastbase_2_world_tf_.getRotation().getW();
...
void Node::initialPoseCallback(const geometry_msgs::PoseWithCovarianceStampedConstPtr& msg)
{
  ...
  double init_x = msg->pose.pose.position.x;
  double init_y = msg->pose.pose.position.y;
  double init_a = yaw;
  ROS_INFO("Setting pose: x %.3f, y %.3f, theta %.3f, [frame=%s]", 
           init_x, init_y, init_a, msg->header.frame_id.c_str());
  
  tf::Transform init_pose;
  init_pose.setOrigin(tf::Vector3(init_x, init_y, 0.0));
  init_pose.setRotation(tf::Quaternion(0.0, 0.0, sin(init_a * 0.5), cos(init_a * 0.5)));
  setInitialPose(init_pose, parameters_.init_x_dev_, parameters_.init_y_dev_, 
                 parameters_.init_z_dev_, parameters_.init_a_dev_);
}
